...
#         password = "Parvez.@12"
#
#         # Directly login without session management
#         print("Logging in...")
#         L.login(username, password)  # Login using username and password
#
...
#         L = login_instaloader()
#         if L is None:
#             raise ValueError("Login failed. Unable to fetch Instagram data.")
#
#         # Parse the URL to extract the post short code
#         parsed_url = urlparse(post_url)
#         path_parts = parsed_url.path.strip('/').split('/')
#
#         # Check if it's a valid Instagram post URL
...
#             'likes': post.likes,
#             'comments': post.comments,
#             'type': post.typename,  # 'GraphImage', 'GraphVideo', or 'GraphSidecar'
#             'post_url': post.url,
#             'media_urls': []  # To store all media URLs
#         }
#
...
#
#
# # Helper functions to call each fetcher
# def get_url_insta(post_url):
#     return fetch_instagram_info(post_url)
#
# def get_story_insta(username):
